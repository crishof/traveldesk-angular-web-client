...
| 13 | **Normalización visual** — 🟡 **Parcial (Bloque 3, commits `64fc8f0d`, `5cae899b`, `d20e6c6c`)**: definidos tokens `brand`(cyan)/`danger`/`success`/`warning`; **acento unificado a cyan** en toda la app (violet/purple de clientes/equipo y emerald de proveedores → `brand`); **error red→`danger`** y **amber→`warning`** (rename pixel-idéntico que centraliza el "rojo de error en 3 tonos"). **Pendiente:** aplicar `success` a los verdes de éxito dispersos, estados (hover/focus/disabled/loading) y responsive (8/19 plantillas sin breakpoints). | frontend | 🟡 | — |
