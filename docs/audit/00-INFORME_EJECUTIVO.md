...
| 8 | **Reforzar aislamiento** — 🟡 **Parcial (Bloque 2, commits `02ada02`, `2a704b7`)**: `agency_id` añadido a `account_payments` (V2) **y `tbl_payments` (V3)**, NOT NULL+FK, + test de aislamiento A vs B. **Bloqueante para RLS descubierto:** la app conecta como superusuario `postgres`, que **ignora RLS**; hace falta un rol de aplicación sin `SUPERUSER`/`BYPASSRLS` y cambiar credenciales en dev/prod/Railway antes de aplicar políticas. **Pendiente:** decisión de rol DB + RLS. | backend/DB | 🟡 | — |
...
- **RLS de Postgres — bloqueante de infraestructura descubierto:** la app conecta como **superusuario `postgres`**, y **los superusuarios ignoran RLS** (las políticas no tendrían efecto). Para usar RLS hay que: (1) crear un rol de aplicación **sin** `SUPERUSER` ni `BYPASSRLS` con los grants necesarios; (2) cambiar las credenciales del datasource en dev/prod/**Railway** y `.env`/compose; (3) luego políticas + GUC por transacción. Es una **decisión de infraestructura** (cambio de credenciales en todos los entornos) — pendiente de tu confirmación antes de tocar nada.
- **Pendiente del Bloque 2:** decisión de rol DB + implementación de RLS.
