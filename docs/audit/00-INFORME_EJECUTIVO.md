...
| 13 | **Normalización visual** — 🟡 **Casi completo (Bloque 3, commits `64fc8f0d`→`4a98782d`)**: tokens `brand`/`danger`/`success`/`warning` definidos y aplicados; **acento unificado a cyan** en toda la app; **error→`danger`**, **amber→`warning`**, **verdes de éxito→`success`** (todo pixel-idéntico salvo el acento); base tokenizada (`styles.scss`), **focus-visible global** (accesibilidad), estados de `.form-input`, y **primitivas `.btn-*`** con estados. **Responsive verificado end-to-end** (stack levantado + login + screenshots a 375px de dashboard/clientes/ventas/reservas/proveedores/detalle): **la app ya es responsive** — las listas usan layouts de **tarjetas** (no tablas fijas) y la única `<table>` (reservas en detalle de proveedor) ya tiene `overflow-x`. El "8/19 sin breakpoints" era un heurístico sin rotura real. **Pendiente menor (opcional):** migrar botones existentes a las primitivas `.btn-*` para reducir duplicación. | frontend | 🟢 | — |
